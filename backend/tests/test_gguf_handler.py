...
def test_inspect_file_marks_header_fields_read_only(sample_gguf: Path) -> None:
    info = gguf_handler.inspect_file(sample_gguf, "root1", "sample.gguf")
    by_key = {m.key: m for m in info.metadata}

    # Synthesized header fields are shown but must not be offered for editing.
    assert by_key["GGUF.version"].editable is False
    assert by_key["GGUF.tensor_count"].editable is False
    assert by_key["general.name"].editable is True


@pytest.mark.parametrize(
    "edit",
    [
        MetadataEdit(op=EditOp.SET, key="GGUF.version", value=2, value_type="UINT32"),
        MetadataEdit(op=EditOp.DELETE, key="GGUF.kv_count"),
        MetadataEdit(op=EditOp.RENAME, key="general.name", new_key="GGUF.version"),
    ],
)
def test_perform_edit_rejects_header_field_edits(sample_gguf: Path, edit: MetadataEdit) -> None:
    """`GGUF.*` fields are regenerated by the writer; editing one would add a
    duplicate housekeeping key to the output file."""
    with pytest.raises(gguf_handler.GGUFHandlerError, match="cannot be edited"):
        gguf_handler.perform_edit(
            sample_gguf,
            root_id="root1",
            rel_path="sample.gguf",
            edits=[edit],
            output=OutputOptions(mode=OutputMode.NEW_NAME),
        )
